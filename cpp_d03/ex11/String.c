...
** Last update Sat Jan  7 00:28:18 2017 Carl DEBRAUWERE
...
static void insert_c(String *this, size_t pos, char const *str){
  if (this->str == NULL) return ;
  char *tmp[3];
  tmp[0] = strndup(this->str, pos);
  tmp[1] = strdup(&this->str[pos]);
  free(this->str);
  this->str = NULL;
  this->str = malloc( strlen(tmp[0]) + strlen(tmp[1]) + strlen(str) + 1);
  strcpy(this->str, tmp[0]);
  strcat(this->str, str);
  strcat(this->str, tmp[1]);
}
...
static void insert_s(String *this, size_t pos, String const *str){
  ...
  free(this->str);
  this->str = NULL;
  this->str = malloc( strlen(tmp[0]) + strlen(tmp[1]) + strlen(str->str) + 1);
  strcpy(this->str, tmp[0]);
  strcat(this->str, str->str);
  strcat(this->str, tmp[1]);
}
